// Fond du hero — adapté du composant React "Galaxy" (react-bits, ogl) en JS
// vanilla, en script classique (pas de type="module").
//
// Note file:// : les <script type="module"> sont bloqués par CORS quand la page
// est ouverte en double-clic (origine "null"), même pour un fichier du même
// dossier. On charge donc ce script en classique et on n'utilise le import()
// dynamique (autorisé hors module) que pour la lib ogl, chargée depuis un CDN.
(function () {
  const container = document.getElementById('heroGalaxy');
  ...
  // Réglages ajustés pour rester dans la palette « encre verte » du Carnet d'Ingénieur
  // plutôt que le bleu/violet spatial par défaut du composant d'origine.
  const settings = {
    density: 0.8,
    hueShift: 150,
    speed: 0.35,
  ...
  function loop(t) {
    ...
    program.uniforms.uStarSpeed.value = (t * 0.001 * settings.speed) / 10.0;
